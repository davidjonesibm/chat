...
const reversedResults = computed(() => [...searchResults.value].reverse());
...
<template>
  <div
    class="flex flex-col h-full bg-base-100 border-l border-base-300 pt-safe pr-safe"
    role="search"
  >
    <!-- Header -->
    <div class="flex items-center px-3 py-2 border-b border-base-300">
      <h2 class="text-sm font-semibold flex-1">Search</h2>
    </div>

    <!-- Results -->
    <div class="flex-1 overflow-y-auto flex flex-col justify-end">
      <!-- Loading -->
      <div
      ...
      <!-- Results list -->
      <template v-else>
        <!-- Load more (at top, loads older results) -->
    ...
    <!-- Search input -->
    <div
      class="flex items-center gap-2 p-3 border-t border-base-300 pb-safe-offset-3"
    >
